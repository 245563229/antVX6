        ...
        <el-form label-width="110px" label-suffix="：">
         ...
         <el-form-item label="节点Y轴">
          ...
          <el-form-item label="文本X轴" v-show="nodeType !== 'text-block'">
            <el-input
              v-model="drawerNode.labelRefX"
              placeholder="文本X轴位置"
              @change="(val) => changeLabelPosition(val, 'X')"
            ></el-input>
          </el-form-item>
          <el-form-item label="文本Y轴" v-show="nodeType !== 'text-block'">
            <el-input
              v-model="drawerNode.labelRefY"
              placeholder="文本y轴位置"
              @change="(val) => changeLabelPosition(val, 'Y')"
            ></el-input>
          </el-form-item>
          <el-form-item label="文本整体旋转" v-show="nodeType !== 'text-block'">
            <el-input
              v-model="drawerNode.labelTransform"
              placeholder="文本整体旋转角度"
              @change="changeLabelTransform"
            ></el-input>
          </el-form-item>
          <el-form-item label="文本独立旋转" v-show="nodeType !== 'text-block'">
            <el-input
              v-model="drawerNode.labelRotate"
              placeholder="文本独立旋转角度"
              @change="changeLabelRotate"
            ></el-input>
          </el-form-item>
          ...
          <el-form-item
            label="边框颜色"
            v-show="nodeType !== 'text-block' && nodeType !== 'image'"
          >
            <el-color-picker
          ...
          <el-form-item label="采集项">
            <el-select
              v-model="drawerNode.collectItem"
              placeholder="请选择绑定设备"
              @change="bindCollectItem"
            >
      ...
      <div class="drawer_wrap">
        <el-form label-width="100px" label-suffix="：">
          ...
          <el-form-item label="文本大小">
            <el-slider
              v-model="drawerEdge.edgeTextSize"
              :min="12"
              :max="60"
          ...
          <el-form-item label="文本颜色">
            <el-color-picker
              v-model="drawerEdge.edgeTextColor"
          ...
          <el-form-item label="文本角度">
            <el-slider
              v-model="drawerEdge.edgeTextAngle"
              :min="0"
              :max="360"
              @change="changeEdgeTextAngle"
            ></el-slider>
          </el-form-item>
          <el-form-item label="功能">
            <el-button
...
export default {
  ...
  methods: {
    ...
    changeFontWeight(val) {
      ...
      this.selectCell.attr("label/style/fontWeight", val ? "bold" : "normal");
    },
      ...
      this.selectCell.setLabels([
        {
          attrs: {
            ...
            body: {
              fill: this.graph.background.options.background.color,
            },
          },
          position: {
    ...
    handleNodeTypeAttrs(cell) {
      ...
      if (cell.attrs.body) {
        this.drawerNode.fill =
          cell.attrs.body.fill === "transparent" ? null : cell.attrs.body.fill;
      } else if (cell.attrs.label) {
        this.drawerNode.fill = cell.attrs.label.fill;
      }

      if (this.nodeType !== "text-block") {
        this.drawerNode.labelRefY = cell.attrs.label && cell.attrs.label.refY;
        this.drawerNode.labelRefX = cell.attrs.label && cell.attrs.label.refX;
        ...
        if (cell.attrs.label && cell.attrs.label.transform) {
          ...
          const deg = transform.substring(7, transform.lastIndexOf(")"));
      ...
      if (this.nodeType === "text-block") {
        ...
        this.drawerNode.fontFill = cell.attrs.label && cell.attrs.label.fill;
      }
      ...
      if (cell.shape === "path") {
        ...
        this.drawerNode.stroke = cell.attrs.body && cell.attrs.body.stroke;
      }
    ...
    handleEdgeAttrs(cell) {
      ...
      this.drawerEdge.edgeStyle =
        cell.attrs.line && Number(cell.attrs.line.strokeDasharray) ? 2 : 1;

      this.drawerEdge.edgeColor = cell.attrs.line && cell.attrs.line.stroke;
